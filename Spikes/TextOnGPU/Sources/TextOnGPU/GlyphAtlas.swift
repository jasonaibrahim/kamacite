...
// Glyph cache: rasterize once per (font, glyph, subpixel bucket), pack into
// shelf-allocated 1024² texture pages, sample forever. Grayscale coverage lives
// in r8 pages; color emoji in bgra8 pages. A backing-scale change flushes
// everything — glyph bitmaps are scale-specific.
...
final class GlyphAtlas {
    ...
    struct Key: Hashable {
        ...
        let bucket: Int
        let isColor: Bool
        /// CoreGraphics adjusts text coverage by polarity: dark-on-light strokes
        /// come out thinner than a white-on-black mask suggests. Rasterizing each
        /// polarity the way CG will actually composite it gives parity by
        /// construction (measured: −14.5 mean luminance bias without this).
    ...
    /// Value is nil for glyphs with no ink (spaces, newlines) — cached so we
    /// don't re-measure them every frame.
    ...
    private func rasterize(
        ...
        // Stem darkening, as AppKit applies to text. Without it our strokes
        // come out visibly thinner than NSTextView's.
        context.setAllowsFontSmoothing(true)
        context.setShouldSmoothFonts(true)
        // The whole point of the buckets: let CoreText place ink at the
        // fractional offset instead of snapping it to the pixel grid.
        ...
        if !isColor {
            if darkOnLight {
                // Draw the way CG will composite it — black ink on a white
                // ground — then invert to coverage. This bakes in CG's
                // polarity-specific coverage adjustment.
        ...
        guard let slot = allocateSlot(width: width, height: height, isColor: isColor) else {
            return nil // glyph larger than a page — not expected in this spike
        }
